...
#read in data set
d = pd.read_csv("ACR_shape_names_long.csv")
...
for category in categoryNames:
    ...
    for response in d.loc[d["image"]==category,"response"]:
...
df=df[colNames]
#rename one element
df['image'] = df['image'].replace({'pt24_02_high': 'pt24.02_high'})

#write to csv
df.to_csv('shape_nameability.csv',index=False)
